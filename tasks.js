...
export const get_token_and_task_data = async (task_name) => {
  await get_token(task_name);
  return await get_task_data();
};

export const send_answer = async (answer, token = tasks_config.token) => {
  try {
...
export const get_task_data_using_question = async (
  token = tasks_config.token,
  question
) => {
  try {
